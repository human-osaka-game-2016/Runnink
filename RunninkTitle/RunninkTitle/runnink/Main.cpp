...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	//追加点
	RoadTex("T,H.png", &g_pGameTexture[TITLE_TEX]);//タイトルの背景
	RoadTex("C,03,P.png", &g_pGameTexture[PUSHENTER_TEXT]);//PUSHENTERの文字
	RoadTex("T,01,R,01.png", &g_pGameTexture[TITLE_TEXT]);//タイトルの文字
	RoadTex("white.png", &g_pGameTexture[HIT_FLG]);//衝突判定（画面外）
	RoadTex("G,02,G,01.png", &g_pGameTexture[GAMEOVER_TEXT]);//ゲームオーバーの文字
	RoadTex("G,02,G,02.png", &g_pGameTexture[PLAYER_DEATH_TEX]);//プレイヤー死亡
	//ここまで
